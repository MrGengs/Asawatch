...
static void ukur_selesai(bool lengkap) {
  ...
  /* Kalibrasi glukosa: offset lab per-unit diset lewat konsol serial
   * ("glu_offset N"), BUKAN BLE -- lihat aw_store.h. Sama seperti tensi di
   * atas: diterapkan ke pembacaan mentah, sentinel 0 ("gagal diukur") tidak
   * disentuh, dan hasilnya diklem supaya offset ekstrem tidak membuatnya
   * membungkus lewat 0/65535. */
  if (aw_kalibrasi_glukosa_ada() && s_acc_gula) {
    long g = (long)s_acc_gula + aw_kalibrasi_glukosa_get();
    s_acc_gula = (uint16_t)constrain(g, 1L, 65535L);
  }

...
/* Kalibrasi glukosa -- kunci NVS TERPISAH dari kalib_t tensi di atas (bukan
 * cuma field tambahan di blob yang sama): keduanya diset lewat jalur yang
 * beda sama sekali (BLE vs konsol serial, lihat aw_store.h) dan mencampurnya
 * di satu blob berarti menulis salah satu ikut menulis ulang yang lain. */
typedef struct {
  uint8_t valid;
  int16_t offset_gula;
} kalib_gl_t;
...
void aw_store_begin(void) {
  ...
  memset(&s_kalib_gl, 0, sizeof(s_kalib_gl));
  ...
  size_t nkg = s_nvs.getBytes(K_KALIB_GL, &s_kalib_gl, sizeof(s_kalib_gl));
  if (nkg != sizeof(s_kalib_gl)) memset(&s_kalib_gl, 0, sizeof(s_kalib_gl));
...
/* ---------------- Kalibrasi glukosa (konsol serial saja) ---------------- */
bool aw_kalibrasi_glukosa_ada(void) { return s_kalib_gl.valid != 0; }

int16_t aw_kalibrasi_glukosa_get(void) {
  return s_kalib_gl.valid ? s_kalib_gl.offset_gula : 0;
}

void aw_kalibrasi_glukosa_set(int16_t offset_mgdl) {
  s_kalib_gl.valid = 1;
  s_kalib_gl.offset_gula = offset_mgdl;
  if (s_nvs_ok) s_nvs.putBytes(K_KALIB_GL, &s_kalib_gl, sizeof(s_kalib_gl));
  Serial.printf("[store] kalibrasi glukosa disimpan: %+d mg/dL\n",
                (int)offset_mgdl);
}

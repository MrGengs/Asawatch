...
static void refresh_cb(lv_timer_t *tm) {
   ...
   * sengaja jarang; pwr_matikan() menyimpan sekali lagi saat dimatikan. */
  if (battery_valid()) {
  ...
  if (battery_valid()) {
     ...
     *   2. Begitu plateau (fase CV) terdeteksi, persen langsung dari tegangan
     *      mentah (lihat battery.cpp) -- jadi 100% di sini bukan sekadar
     *      lonjakan sesaat, aman dijadikan syarat mati permanen.
    ...
    if (kotak != kotak_lalu || chg != isi_lalu) {
       ...
       * mengharapkan jam langsung menunjukkan sedang mengisi.
       *
...
static void konsol_jalankan(char *baris) {
  ...
  else if (!strcmp(baris, "glu_offset")) {
    /* Offset kalibrasi glukosa per-unit -- konsol saja, bukan protokol BLE
     * (lihat komentar aw_kalibrasi_glukosa_set di aw_store.h). Ditambahkan ke
     * pembacaan mentah tepat sekali di ukur_selesai() (aw_jam.cpp), sama
     * seperti kalibrasi tensi. Nilai NEGATIF ditulis "-N", mis. "glu_offset
     * -8" kalau Asawatch cenderung lebih tinggi daripada acuan. */
    if (sp) aw_kalibrasi_glukosa_set((int16_t)arg);
    else Serial.printf("[konsol] pakai: glu_offset <+-N mg/dL>  (sekarang %+d)\n",
                        (int)aw_kalibrasi_glukosa_get());
  }

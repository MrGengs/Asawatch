...
Sketch Arduino untuk **ESP32-C6-LCD-1.69, varian TANPA sentuh** (240x280,
ST7789V2 saja — tidak ada CST816T atau chip sentuh lain di board ini). Jam ini
adalah **sensor + buffer + penampil**: ia mengukur PPG, menyimpan hasilnya di
ring buffer NVS, dan mengirimkannya ke aplikasi Flutter lewat BLE. Semua
kecerdasan (verdict, tren, penjadwalan sesi) ada di aplikasi, bukan di sini.

Direktori ini adalah **firmware yang aktif** untuk varian tanpa sentuh. Ada
sketch kembar untuk varian layar sentuh di `../touchscreen` (folder tetangga,
`Asawatch/touchscreen`) — jangan campur adukkan perubahan antara keduanya
tanpa sengaja; keduanya sudah berpisah sejak titik ini dan boleh berbeda
...
arduino-cli compile -b "$FQBN" --build-path "$BUILD" "/home/rad/project/Project Enuma/arduino/no_touch"
arduino-cli upload  -b "$FQBN" -p /dev/ttyACM0 --input-dir "$BUILD" "/home/rad/project/Project Enuma/arduino/no_touch"
```
...
```
no_touch.ino       UI LVGL satu halaman, pin map, tombol PWR/BOOT, setup/loop,
...
**Konteks loop()** — `lv_timer_handler`, `jam_putar`, seluruh akses I2C (RTC,
MAX30105), seluruh akses flash/NVS.
...
Bus I2C 100 kHz, **bukan** 400 kHz. Board ini tidak punya chip sentuh (alasan
historis kecepatan ini di varian sentuh adalah CST816T), tapi 400 kHz belum
pernah diuji di board ini untuk kombinasi RTC PCF85063 + PPG MAX3010x, jadi
kecepatannya dipertahankan konservatif sampai ada pengukuran yang membuktikan
sebaliknya. `ppg.cpp` karena itu tidak boleh memanggil
`begin(Wire, I2C_SPEED_FAST)`.
...
- **Urutan init `setup()`**: latch `BAT_EN` HIGH paling awal (tanpa itu board
  mati begitu jari lepas dari tombol PWR) → gerbang tahan-3-detik → `Wire.begin()`
  → `gfx->begin()` → LVGL (tanpa indev, board ini tidak punya chip sentuh) →
  sensor → splash → `jam_mulai()` → `net_begin()` (AsaWatch dulu baru Wi-Fi,
  demi heap). Urutan touch-sebelum-gfx dari varian sentuh TIDAK berlaku di sini
  — tidak ada apa pun untuk diinisialisasi lebih dulu.
...
Konsol serial di `no_touch.ino` menyuntik opcode **lewat antrean yang sama** dengan
...
lihat komentar di `no_touch.ino`) adalah keluaran skrip di `scripts/`:

...
bawah; bila kode firmware dan dokumen ini berbeda, kodenya yang bug. Versi kawat: **v1.2**.
...
`uptime_s`-nya sendiri saat perintah tiba, pindah ARMED → RUNNING, mengukur index 1, menjadwalkan
`+1 jam`/`+2 jam`, lalu mengirim `TOMBOL_SELESAI_MAKAN` — persis seperti kalau tombol fisiknya yang
ditekan.

Kenapa bukan aplikasi yang mengirim `t0`-nya sendiri: jam tidak punya RTC (§2), jadi satu-satunya
`t0` yang bisa dibandingkan dengan `uptime_s` sampel-sampelnya adalah yang berasal dari pencacah
yang sama. Kalau suatu saat ada yang tergoda menaruh epoch di payload ini, seluruh §2 runtuh.
...
Ia **tidak menyentuh mesin status** (§12): tidak memindahkan status, tidak menggeser jadwal titik
ukur sesi, dan tidak menghabiskan index. Hasilnya bukan bagian dari sesi makan mana pun — `sesiId`
...
- **IDLE** — tombol "Selesai Makan" tidak berfungsi, dan `MULAI_SESI` dijawab NAK `0x03`.
  `UKUR_SEKARANG` tetap boleh.
- **ARMED** — tombol aktif, dan `MULAI_SESI` (§5) melakukan hal yang sama persis dengannya. `UKUR`
  index 0 (baseline) dilayani. **Timeout 4 jam** `uptime_s`: lewat
...
konfirmasi aplikasi untuk berpindah status — `MULAI_SESI` adalah cara kedua menekan tombol yang
sama, bukan pengganti jadwal jam sendiri.
...
| Titik ukur sesi jatuh tempo | `UKUR_SEKARANG` berjalan | **Ditunda, diambil segera setelah sensor bebas.** Jangan dibatalkan, dan jangan biarkan pengukuran yang sedang jalan menabraknya |
| `UKUR_SEKARANG` | titik ukur sesi berjalan | NAK `0x05`; aplikasi mengulang setelah 5 detik. **Satu-satunya yang boleh ditolak** |
| `UKUR` index 0 (baseline) | apa pun | NAK `0x05`; aplikasi tidak mengulang selamanya dan menandai baseline terlewat |
...
**Bentuk kode yang membuat penundaan aman.** Pakai perbandingan `uptime_s` absolut dengan bitmask
index yang baru menyala **setelah** pengukuran tuntas, plus satu penjaga "sedang mengukur" — titik
yang terlewati satu putaran otomatis dicoba lagi di putaran berikutnya, bukan hilang:

```c
if (sedang_mengukur) return;            // titik ukur ditunda, tidak dibatalkan

if (!(index_selesai & (1 << 1))) { ukur(1); return; }   // segera setelah t0
if (!(index_selesai & (1 << 2)) && sekarang >= t0_uptime + 3600) ukur(2);
if (!(index_selesai & (1 << 3)) && sekarang >= t0_uptime + 7200) { ukur(3); ke_idle(); }
```
...
selesai seketika — dan itu justru alasan menulisnya sekarang, bukan nanti. Tanpa penjaga itu, titik
ukur sesi akan menabrak `UKUR_SEKARANG` yang sedang berjalan pada hari pertama sensor sungguhan
terpasang, jauh dari perubahan yang menyebabkannya.
...
- [ ] `UKUR_SEKARANG` dilayani di **ketiga status** dan tidak menggeser jadwal titik ukur sesi
